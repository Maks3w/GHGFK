          ...
          repository.getPr(pr.number).then(function (pr) {
            ...
            githubLoggedUserService.getUser().then(function (user) {
              if (pr.assignee === null) {
                ...
                $rootScope.$broadcast('alert.new', 'error', `#${pr.number} cannot be merged while still assigned to ${pr.assignee.login}`);
                return;
              }
      ...
      template: `
        <div>
          ...
          <div ng-hide="isCollapsed">
            ...
            <gh-branch-create-menu bases="selectedBranches" name="localPrBranch" repo="pr.base.repo.full_name">
            </gh-branch-create-menu>
            <!-- merge PR in local branch button -->
            <button class="btn btn-warning gh-merge" repo="{{pr.base.repo.full_name}}" base="{{localPrBranch}}"
             head="{{pr.originalElement.head.sha}}" msg="{{ commitMsg }}"
             title="Merge PR #{{ pr.number }} in {{ localPrBranch }} with message {{ commitMsg }}">
              <span class="type-icon octicon octicon-git-merge"></span>
              #{{ pr.number }} in {{ localPrBranch }}
            </button>
            <a class="btn" href="{{ pr.base.repo.html_url }}/tree/{{ localPrBranch  }}" target="_blank">
              Browse files and edit
            </a>
            <!-- Merge PR in final branches button -->
            <gh-merge-menu bases="selectedBranches" head="localPrBranch" repo="pr.base.repo.full_name" msg="commitMsg">
            </gh-merge-menu>
            <!-- delete local branch button -->
            <button class="btn btn-danger gh-branch-delete" repo="{{pr.base.repo.full_name}}" name="{{localPrBranch}}"
             title="Delete {{ localPrBranch }}">

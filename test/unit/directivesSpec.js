    ...
    it('should show merge options', function () {
      ...
      inject(function ($compile, $rootScope) {
        ...
        var element = $compile('<merge-button pr="pr"/>')($rootScope);
          $rootScope.$digest();
        expect(element.html()).toEqual('<label>Patch type<select class="ng-pristine ng-valid ng-valid-required" ng-model="patchType" required=""><option value="">Select patch type</option><option value="hotfix">Hotfix</option><option value="feature">Feature</option></select></label><label>Commit msg <input class="ng-pristine ng-binding ng-valid ng-valid-required" ng-model="commitMsg" ng-bind-template="Merge PR #959" required="" type="text"></label><div class="form-group container"><!-- ngRepeat: (branch, merge) in branches --><label ng-repeat="(branch, merge) in branches" class="checkbox inline ng-scope ng-binding"><input checked="checked" class="ng-pristine ng-valid" value="develop" ng-model="branches[branch]" ng-checked="merge" type="checkbox">  develop</label><label ng-repeat="(branch, merge) in branches" class="checkbox inline ng-scope ng-binding"><input checked="checked" class="ng-pristine ng-valid" value="master" ng-model="branches[branch]" ng-checked="merge" type="checkbox">  master</label></div><input class="btn btn-danger" value="Merge" ng-disabled="mergeFrm.$invalid" type="submit">');
      });
    });
  });
});
